...
- 源码可支撑的结论：必须引用**有版本管理的代码仓库**（含仓内文档）；
  不引用无版本管理的官网页面。
- **来源质量门槛**：引用前先确认来源权威性——官方文档/官方源码优先；
  低星个人仓库、作者背景不明的博客不得作为依据；顺手搜到 ≠ 可引用。
...
## 专项约定：dsh-known-truth

以下编写规范适用于 `skills/dsh-known-truth/`，优先于上面的通用条款：
...
- **版本基准**：当前 `0.1.0-rc.6`（SKILL.md 开头与本处保持同步）。
  dsh 升级后按 `references/sources.md` 逐条复核：内容变化则更新并提升
  基准；复核无变化也更新基准版本号（表示"已在此版本验证"）。
- **引用格式**：一律 `github.com/deepseek-ai/deepseek-harness` 的
  blob/tree 链接，行级结论附 `#L` 锚点；仓库无 tag，按 `master` 路径，
  漂移核对方式在 `references/sources.md` 说明。

## 提交前验证清单

1. 每个 `SKILL.md` 以 `---` YAML 块开头，frontmatter 字段齐全；
2. 正文引用的相对路径文件全部存在；
3. `references/` 中的仓库链接可访问且来源权威（抽查）；
4. 无第三方项目名残留；无应放仓库级的辅助文档混入 skill 目录；
5. 新增规则符合 3 行篇幅纪律；本次内容已经过用户清单审查。
